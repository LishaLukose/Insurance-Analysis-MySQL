...
#KPI-1 No of Invoice Generated by Account Executive
...
#KPI-3.1 Cross Sell (Target, Achieved, Invoice)
select 
...
#KPI-3.2 New (Target, Achieved, Invoice)
select 
...
#KPI-3.3 Renewal (Target, Achieved, Invoice)
select 
...
#KPI-6 Open opportunity - Top 4
select opportunity_name, CONCAT(format(SUM(revenue_amount) / 1000, 0), 'K') as Revenue from opportunity
where stage in ('Qualify Opportunity','Propose Solution') group by opportunity_name order by SUM(revenue_amount) desc limit 4;
